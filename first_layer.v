module first_layer(
    ...
    enable_register_write // Removi a virgula extra aqui
);
    ...
    // ==========================================
    // MÁQUINA DE ESTADOS 1 (FR_FMS) - SEQUENCIAL
    // ==========================================
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            case (fr_fms_state)
                RQ_PIXEL: if (is_avaliable_pixel) fr_fms_state <= ST_PIXEL;
                ST_PIXEL: begin
                    data_a_pixel <= data_in_pixel; // Salva o dado
                    if (is_avaliable_pixel) fr_fms_state <= ST_PIXEL;
                    else fr_fms_state <= RQ_WIN0;
                end
                RQ_WIN0: if (is_avaliable_win) fr_fms_state <= ST_WIN0;
                ST_WIN0: begin
                    data_b_win[0] <= data_in_win; // Salva o dado
                    if (is_avaliable_win) fr_fms_state <= ST_WIN0;
                    else fr_fms_state <= RQ_WIN1;
                end
                RQ_WIN1: if (is_avaliable_win) fr_fms_state <= ST_WIN1;
                ST_WIN1: begin
                    data_b_win[1] <= data_in_win; // Salva o dado
                    if (is_avaliable_win) fr_fms_state <= ST_WIN1;
                    else fr_fms_state <= RQ_WIN2;
                end
                RQ_WIN2: if (is_avaliable_win) fr_fms_state <= ST_WIN2;
                ST_WIN2: begin
                    data_b_win[2] <= data_in_win; // Salva o dado
                    if (is_avaliable_win) fr_fms_state <= ST_WIN2;
                    else fr_fms_state <= RQ_WIN3;
                end
                RQ_WIN3: if (is_avaliable_win) fr_fms_state <= ST_WIN3;
                ST_WIN3: begin
                    data_b_win[3] <= data_in_win; // Salva o dado
                    if (is_avaliable_win) fr_fms_state <= ST_WIN3;
                    else fr_fms_state <= CALC;
                end
                CALC: fr_fms_state <= WAIT_SD_FMS;
                WAIT_SD_FMS: begin
                    if (iteration_done) fr_fms_state <= RQ_PIXEL;
                end
                default: fr_fms_state <= RQ_PIXEL;
            endcase 
        end
    end
    
    // ==========================================
    // LÓGICA COMBINACIONAL 1 (FR_FMS)
    // ==========================================
    always @(*) begin
        // DEFAULT: Elimina latches!
        enable_sd_fms = 1'b0;
        req_win = 1'b0;
        req_pixel = 1'b0;
    ...
    // ==========================================
    // MÁQUINA DE ESTADOS 2 (SD_FMS) - SEQUENCIAL
    // ==========================================
    always @(posedge clk or posedge rst) begin
        if (rst) begin
    ...
    // ==========================================
    // LÓGICA COMBINACIONAL 2 (SD_FMS)
    // ==========================================
    always @(*) begin
        // DEFAULT: O SALVADOR DA PÁTRIA! Desliga o MAC instantaneamente.
        iteration_done = 1'b0;
        req_bias = 1'b0;
        enable_register_write = 1'b0;
        enable_nodes_by_sd_fms = 1'b0; // <--- A CORREÇÃO DO SEU BUG
...
/*module first_layer(
	...
	always @(posedge clk or posedge rst) begin
		if (rst) begin
			...
			case (sd_fms_state)
				VF_LAST_PIXEL: begin
					if (pixel_counter == 10'd783) begin
						if (is_avaliable_bias) begin
							sd_fms_state <= VF_LAST_PIXEL;
						end else begin
							sd_fms_state <= RQ_BIAS0;
						end
					end else begin
				...
				ST_BIAS0: begin
					if (is_avaliable_bias) begin
						...
						sd_fms_state <= RQ_BIAS1;
					end
				end
				RQ_BIAS1: begin
					if (is_avaliable_bias) begin
						sd_fms_state <= ST_BIAS1;
					end else begin
						sd_fms_state <= RQ_BIAS1;
					end
				end
				ST_BIAS1: begin
					if (is_avaliable_bias) begin
						...
						sd_fms_state <= RQ_BIAS2;
					end
				end
				RQ_BIAS2: begin
					if (is_avaliable_bias) begin
						sd_fms_state <= ST_BIAS2;
					end else begin
						sd_fms_state <= RQ_BIAS2;
					end
				end
				ST_BIAS2: begin
					if (is_avaliable_bias) begin
						...
						sd_fms_state <= RQ_BIAS3;
					end
				end
				RQ_BIAS3: begin
					if (is_avaliable_bias) begin
						sd_fms_state <= ST_BIAS3;
					end else begin
						sd_fms_state <= RQ_BIAS3;
					end
				end
				ST_BIAS3: begin
					if (is_avaliable_bias) begin
						...
						sd_fms_state <= SUM_BIAS;
					end
				end
				SUM_BIAS: begin
					sd_fms_state <= ST_RES0;
				end
				ST_RES0: begin
					sd_fms_state <= WAIT_RES0;
				end
				WAIT_RES0: begin
				...
				INCREMENT: begin
					if (iteration_counter == 5'd31 && pixel_counter == 10'd783) begin
						done <= 1'b1;
					end else begin
						done <= 1'b0;
					end
	...
	always @(*) begin
		...
		case (sd_fms_state)
			...
			ST_RES0: begin
				addr_to_raw_in_register = (iteration_counter << 2);
				data_to_raw_in_register = data_out_nodes[0];//data_out_after_activation[0];
				enable_register_write = 1'b1;
			end
			WAIT_RES0: begin
				addr_to_raw_in_register = (iteration_counter << 2);
			...
			ST_RES1: begin
				addr_to_raw_in_register = (iteration_counter << 2) + 1;
				data_to_raw_in_register = data_out_nodes[1];//data_out_after_activation[1];
				enable_register_write = 1'b1;
			end
			WAIT_RES1: begin
				addr_to_raw_in_register = (iteration_counter << 2) + 1;
			...
			ST_RES2: begin
				addr_to_raw_in_register = (iteration_counter << 2) + 2;
				data_to_raw_in_register = data_out_nodes[2];//data_out_after_activation[2];
				enable_register_write = 1'b1;
			end
			WAIT_RES2: begin
				addr_to_raw_in_register = (iteration_counter << 2) + 2;
			...
			ST_RES3: begin
				addr_to_raw_in_register = (iteration_counter << 2) + 3;
				data_to_raw_in_register = data_out_nodes[3];//data_out_after_activation[3];
				enable_register_write = 1'b1;
			end
			WAIT_RES3: begin
				addr_to_raw_in_register = (iteration_counter << 2) + 3;
